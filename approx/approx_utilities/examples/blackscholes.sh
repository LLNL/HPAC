...
python ./benchmark.py -b -i build/blackscholes.yaml -t "approx_space/$exp_size/approximate_techniques.yaml" -r $repeat -n 1,16,32  
python analysis_scripts/concatenate.py analysis_results/ blackscholes

if [ "$visualize" == true ]; then
  python analysis_scripts/viz_blackscholes.py analysis_results/blackscholes.pkl 
fi
