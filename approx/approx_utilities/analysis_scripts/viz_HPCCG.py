...
def scatterPlotOnly(appName, data, output_file, x_name, y_name, hue_col, style_col, v_line=None, h_line=None, x_cut = None, y_cut = None):
    ...
    local_data = local_data[local_data[x_name] < 10]
    ...
    regions = local_data[hue_col].unique()
    approximation_colors = {}
    markers ={}
    allMarkers=[["d","none"], ["s","none"], ["v","none"], ["+", "fill"], ["x", "fill"]]
    colors ={}
    tmp_colors2 = sns.color_palette("mako",len(regions))
    ...
    for r in regions:
        data= local_data[local_data[hue_col] == r]
        x= data[x_name].tolist()
        y= data[y_name].to_list()
        if ( markers[r][1] == "none"):
            ax.scatter(x, y, linewidth=1.0, color=colors[r], marker=markers[r][0], edgecolors=colors[r], facecolors='none')
        else:
            ax.scatter(x, y, linewidth=1.0, color=colors[r], marker=markers[r][0], edgecolors=colors[r])
    ...
    legend = ax.legend(handles = legends, title="Region", frameon= False)
    plt.tight_layout()
...
if (len(sys.argv) != 2):
    ...
    print ("%s input_file applicationName outputDirectory" %sys.argv[0])
    sys.exit(0)
...
scatterPlotOnly(print_names[appName], approx, outPrefix + "HPCCG_figure5.pdf", "Speed Up", error_type[appName], "Region", "Num Threads", cut_perf, None)
