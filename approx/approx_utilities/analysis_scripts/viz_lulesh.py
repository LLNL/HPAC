...
def scatterPlotOnly(appName, data, output_file, x_name, y_name, hue_col, style_col, v_line=None, h_line=None, x_cut = None, y_cut = None):
    ...
    for t in approxTechniques:
        legends.append(Line2D([0], [0], lw=0.2, marker=markers[t][0], color=colors[t], markerfacecolor="white",  mec=colors[t], label=t, linestyle='None'))
    ...
    for t in approxTechniques:
        ...
        if ( markers[t][1] == "none"):
            ax.scatter(x, y, linewidth=0.8, s=8, color=colors[t], marker=markers[t][0], edgecolors=colors[t], facecolors='none')
        else:
            ax.scatter(x, y, linewidth=0.8, s=8, color=colors[t], marker=markers[t][0], edgecolors=colors[t])
...
#Grouping and mean values are computed on concat script
#df.groupby(indexes).apply(something)#.mean().reset_index()
approx = df
...
#processed=[]
#
#for p in accurate["Num Threads"].unique():
#    accTime = accurate[accurate["Num Threads"] == p]["Exec. Time"].mean()#values[0]
#    stdTime = accurate[accurate["Num Threads"] == p]["Exec. Time"].std()#values[0]
#    length= len(accurate[accurate["Num Threads"] == p]["Exec. Time"])#values[0]
#    print(appName,p, stdTime/accTime*100.0, length)
##    print(p,accTime,stdTime,length)
##    print(p,mean_confidence_interval(accTime), mean_confidence_interval(accTime, 0.99))
#    continue
#    tmp= approx[approx["Num Threads"] == p]
#    tmp["Norm. Time"] = tmp["Exec. Time"] / accTime
#    processed.append(tmp)
#
#sys.exit(0)

def TAFPlot(data, x_axis, y_axis, column_axis, output_file):
    sizes=set_size(width=textWidth, fraction=0.48)
    vals_sizes=get_width_height(width=textWidth, fraction=0.48)
    height = vals_sizes[1]
    aspect =  vals_sizes[0]/height
    aspect = aspect/2
    print(vals_sizes)
    fig, ax = plt.subplots(figsize=sizes)
    plot = sns.catplot(x=x_axis, y=y_axis, height=height, aspect=aspect,
            hue = "Technique", legend=False, 
             col=column_axis,
            data=data, kind="violin")
#            height=4, aspect=.7);
    plt.margins(x=0.0, y=0.0)
    plt.tight_layout()
    plot.fig.savefig(output_file,bbox_inches='tight')
    plt.close()
...
scatterPlotOnly(print_names[appName], approx, outPrefix + appName + "_figure8.pdf", "Speed Up", error_type[appName], "Technique", "Region",1.0,None, 2.0)
