...
private:
  ...
  MemoTable(int NumEntries, ValType *tab_ptr, ValType _num_items_per_entry, char *_InputIdx, int *_clock_indexes, unsigned char *_ReplacementData, TableReplacementPolicy *_RP){
    ...
    ValType calc_distance(approx_var_info_t &in_var, int entry, int offset)
    {
        ...
        for(int i = 0; i < in_var.num_elem; i++)
        {
  ...
  void add_entry(approx_var_info_t *inputs, int nInputs)
  {
    int idx_offset = 0;
    int offset = 0;
